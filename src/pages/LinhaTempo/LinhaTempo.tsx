...
const LinhaTempo: React.FC = () => {
  ...
  useEffect(() => {
    if (apiData?.values) {
      ...
      // Extrair veículos únicos - corrigindo o erro do TypeScript
  ...
  const filteredData = useMemo(() => {
    if (selectedVehicles.length === 0) {
      // Se nenhum veículo estiver selecionado, mostrar todos
  ...
  const chartData: ChartData[] = useMemo(() => {
    // Agrupar por data e somar impressões
  ...
  const totalClicks = filteredData.reduce((sum, item) => sum + (item.linkClicks || 0), 0)
                ...
                tooltip={({ point }) => (
                  <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
                    ...
                    <div className="text-sm text-gray-600">
                      Impressões: {(point.data.y as number).toLocaleString("pt-BR")}
                    </div>
                  </div>
                )}
              />
